...
_G._FH_NOVAMOON_GEN = (_G._FH_NOVAMOON_GEN or 0) + 1
local _GEN = _G._FH_NOVAMOON_GEN
_G._FH_SHUTDOWN = false
_G._FH_SHUTDOWN_DONE = false
local function _isShuttingDown()
    if _G._FH_SHUTDOWN then return true end
    if _GEN ~= _G._FH_NOVAMOON_GEN then return true end
    local lp = Players.LocalPlayer
...
pcall(function()
    ...
    local function purge(parent)
        ...
        for _, ch in ipairs(parent:GetChildren()) do
            if ch:IsA("ScreenGui") and ch.Name == "NovaMoonHub" then
    ...
    if _G._FH_NOVAMOON_GUI then pcall(function() _G._FH_NOVAMOON_GUI:Destroy() end) end
end)
_G._FH_NOVAMOON_GUI = nil
...
local T = {
    ...
    BgDeep      = Color3.fromRGB(25,  28,  35),
    Side        = Color3.fromRGB(35,  38,  45),
    SideHover   = Color3.fromRGB(45,  48,  58),
    SideActive  = Color3.fromRGB(55,  58,  68),
    Card        = Color3.fromRGB(25,  28,  35),
    CardHover   = Color3.fromRGB(35,  38,  45),
    Line        = Color3.fromRGB(80,  85,  95),
    Soft        = Color3.fromRGB(40,  43,  52),
    Text        = Color3.fromRGB(255, 255, 255),
    TextDim     = Color3.fromRGB(140, 145, 155),
    TextMute    = Color3.fromRGB(100, 105, 115),
...
local Theme = {
    c1 = Color3.fromRGB(88, 101, 242),
    c2 = Color3.fromRGB(60,  70,  160),
}
local ThemeGrads  = {}
local ThemeSolids   = {}
local ThemeBgFrames = {}
...
_G._FH_NOVAMOON_GUI = GUI

-- Auto-copy Discord link to clipboard on load
pcall(function()
    local clipText = "https://discord.gg/8ubwDFwT"
    if type(setclipboard) == "function" then
        setclipboard(clipText)
    elseif syn and syn.set_clipboard then
        syn.set_clipboard(clipText)
    elseif type(copyclipboard) == "function" then
        copyclipboard(clipText)
    elseif type(Clipboard) == "table" and Clipboard.set then
        Clipboard.set(clipText)
    end
end)

-- Discord link notification toast
pcall(function()
    task.delay(0.5, function()
        local notifGui = Instance.new("ScreenGui")
        notifGui.Name = "NovaMoonHub_DiscordNotif"
        notifGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
        notifGui.ResetOnSpawn = false
        notifGui.IgnoreGuiInset = true
        notifGui.DisplayOrder = 999999
        pcall(function()
            if typeof(gethui) == "function" then notifGui.Parent = gethui()
            else notifGui.Parent = game:GetService("CoreGui") end
        end)
        if not notifGui.Parent then
            pcall(function() notifGui.Parent = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui") end)
        end
        ...
        link.Size = UDim2.new(1, -55, 0, 14)
        link.Position = UDim2.new(0, 45, 0, 26)
        link.BackgroundTransparency = 1
        link.Text = "https://discord.gg/8ubwDFwT"
        link.Font = Enum.Font.Gotham
        link.TextSize = 10
        link.TextColor3 = Color3.fromRGB(140, 145, 155)
        link.TextXAlignment = Enum.TextXAlignment.Left
        link.ZIndex = 999999
        link.Parent = frame

        -- Animate in
        frame:TweenPosition(UDim2.new(0.5, -140, 0, 16), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.5, true)

        -- Auto remove after 5 seconds
        task.delay(5, function()
            pcall(function()
                frame:TweenPosition(UDim2.new(0.5, -140, 0, -60), Enum.EasingDirection.In, Enum.EasingStyle.Quart, 0.4, true)
                task.wait(0.5)
                notifGui:Destroy()
            end)
        end)
...
(function()
    ...
    local bnLogoLetter = Lbl(LogoBox, "N", isMobile and 15 or 24, T.White, Enum.Font.GothamBlack, Enum.TextXAlignment.Center)
    ...
    local function bnDivider(x)
        ...
        d.BackgroundColor3 = Color3.fromRGB(80, 85, 95)
        d.BorderSizePixel  = 0
        d.ZIndex           = 100001
        d.Parent           = banner
    end
    ...
    local HubLabel = Lbl(banner, "NovaMoon Hub", isMobile and 9 or 13, T.White, Enum.Font.GothamBold)
    ...
    local DevLabel = Lbl(banner, isMobile and "NovaMoon Team" or "Credits: NovaMoon Team", isMobile and 8 or 11, Color3.fromRGB(225, 215, 245), Enum.Font.GothamBold)
    ...
    StatusBadge.BackgroundColor3 = Color3.fromRGB(35, 38, 45)
    ...
    local function _safeSaveBanner()
        local cfg = _G._FH_NMBannerCfg or {}
        cfg.y      = banner.Position.Y.Offset
        _G._FH_NMBannerCfg = cfg
        if Config and Config.set then
            pcall(function() Config.set("nm_banner_pos_y",  cfg.y) end)
        end
    end
    ...
    task.spawn(function()
        ...
        local py = Config.get("nm_banner_pos_y", nil)
        if py then
            local p = banner.Position
...
Corner(LogoBadge, 7)
local LogoLetter = Lbl(LogoBadge, "N", isMobile and 13 or 18, T.White, Enum.Font.GothamBlack, Enum.TextXAlignment.Center)
...
local LogoTitle = Lbl(Logo, "NOVA", isMobile and 9 or 11, T.Text, Enum.Font.GothamBold)
LogoTitle.Size     = UDim2.new(1, -38, 0, 13)
LogoTitle.Position = UDim2.new(0, 38, 0, 6)
LogoTitle.ZIndex   = 5
local LogoSub = Lbl(Logo, "MoonHub v3.0", isMobile and 7 or 9, T.TextDim, Enum.Font.GothamMedium)
...
DiscordBtn.Size                   = UDim2.new(1, -14, 0, isMobile and 24 or 30)
DiscordBtn.Position               = UDim2.new(0, 7, 0, isMobile and 52 or 66)
DiscordBtn.BackgroundColor3       = Color3.fromRGB(88, 101, 242)
DiscordBtn.BackgroundTransparency = 0.15
DiscordBtn.BorderSizePixel        = 0
DiscordBtn.Text                   = "Discord"
DiscordBtn.Font                   = Enum.Font.GothamBold
DiscordBtn.TextSize               = isMobile and 9 or 11
DiscordBtn.TextColor3             = Color3.fromRGB(255, 255, 255)
DiscordBtn.AutoButtonColor        = false
DiscordBtn.ZIndex                 = 5
DiscordBtn.Parent                 = Side
Corner(DiscordBtn, 6)
local dbStroke = Stroke(DiscordBtn, Color3.fromRGB(120, 135, 255), 1, 0.3)
DiscordBtn.MouseEnter:Connect(function()
    Tween(DiscordBtn, F, {BackgroundColor3 = Color3.fromRGB(110, 125, 255)})
    Tween(dbStroke, F, {Transparency = 0.1})
end)
DiscordBtn.MouseLeave:Connect(function()
    Tween(DiscordBtn, F, {BackgroundColor3 = Color3.fromRGB(88, 101, 242)})
    Tween(dbStroke, F, {Transparency = 0.3})
end)
DiscordBtn.Activated:Connect(function()
    pcall(function()
        if type(setclipboard) == "function" then
            setclipboard("https://discord.gg/8ubwDFwT")
        elseif type(toclipboard) == "function" then
            toclipboard("https://discord.gg/8ubwDFwT")
        end
    end)
...
NavList.Position               = UDim2.new(0, 7, 0, isMobile and 84 or 102)
...
local Config = {}
do
    local FOLDER      = "NovaMoonHub"
    local FILE        = FOLDER .. "/config.json"
    local FILE_BAK    = FOLDER .. "/config.bak.json"
    local LEGACY_FILE = "novamoon_hub_config.json"
        ...
        or inp.UserInputType == Enum.UserInputType.Touch then
            if pillDragging then
                ...
                if not pillMoved and (tick() - (pillT or 0)) < 0.3 then
                    ...
                    Config.set("nm_reopen_pos", { p.X.Scale, p.X.Offset, p.Y.Scale, p.Y.Offset })
                end
            end
        end
    end)
...
_sbTop.BackgroundColor3 = Color3.fromRGB(80, 85, 95)
...
local _statsConn; _statsConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
    if _GEN ~= _G._FH_NOVAMOON_GEN then _statsConn:Disconnect(); return end
...
do
    _G._FH_SaveLock = function(state) pcall(function() Config.set("nm_gui_locked", state and true or false) end) end
    if Config.get("nm_gui_locked", false) == true then
        guiLocked = true
...
do
    local rp = Config.get("nm_reopen_pos", nil)
    if type(rp) == "table" and #rp == 4 then
        ...
        task.defer(function()
            ...
            if math.abs(_rnx - _absR.X) > 1 or math.abs(_rny - _absR.Y) > 1 then
                ...
                Config.set("nm_reopen_pos", { 0, _rnx, 0, _rny })
            end
        end)
    end
end
do
    ...
    Theme.c1 = loadCol("nm_theme_c1", Theme.c1)
    Theme.c2 = loadCol("nm_theme_c2", Theme.c2)
    local _savedBg = loadCol("nm_theme_bg", nil)
    if _savedBg then
...
do
    local saved = tonumber(Config.get("nm_slider:UI Size", nil))
    if saved then
        task.defer(function()
...
local function attachKeybind(host, nameLbl, baseName, fireFn)
    local entry = {
        ...
        cfgKey = "nm_keybind:" .. baseName, padCfgKey = "nm_padbind:" .. baseName,
    }
...
local function showToast(label, state)
    ...
    toast.BackgroundColor3       = Color3.fromRGB(25, 28, 35)
    ...
    barTrack.BackgroundColor3       = Color3.fromRGB(80, 85, 95)
...
local function Dropdown(parent, name, opts, cb, defaultOpt)
    ...
    do
        local saved = Config.get("nm_dropdown:" .. name, nil)
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
                if _GEN ~= _G._FH_NOVAMOON_GEN then return end
                _brAcc = _brAcc + dt
...
-- Discord Link Billboard Above Head
local DiscordBillboard = (function()
    local M = {}
    local cloneref = cloneref or function(o) return o end
    local Players = cloneref(game:GetService("Players"))
    local lp = Players.LocalPlayer
    ...
    bg.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
    ...
    if lp then
        if lp.Character then update() end
        conn = lp.CharacterAdded:Connect(update)
    end

    M.setVisible = function(v) bb.Enabled = v and true or false end
    return M
end)()
_G._FH_REG_DiscordBillboard = DiscordBillboard
...
local FriendESP = (function()
    ...
    task.spawn(function()
        ...
        local saved = Config.get("nm_friendesp_names", nil)
...
local Booster = (function()
    local M = {
        enabled     = false,
        userEnabled = false,
        speed       = Config.get("nm_booster_spd", 29),
        jump        = Config.get("nm_booster_jmp", 50),
    }
    function M.setSpeed(n) M.speed = tonumber(n) or M.speed; Config.set("nm_booster_spd", M.speed) end
    function M.setJump(n)  M.jump  = tonumber(n) or M.jump;  Config.set("nm_booster_jmp", M.jump)  end
    ...
    function M.set(v)
        ...
        Config.set("nm_booster_on", M.userEnabled)
        applyState()
    end
    ...
    task.spawn(function()
        while true do
            task.wait(1)
            if _GEN == _G._FH_NOVAMOON_GEN then applyState() end
        end
    end)
...
local HideOnEquip = (function()
    ...
    local function apply()
        if not _G._FH_NOVAMOON_GUI then return end
        local shouldShow = not enabled or not isEquipped()
        _G._FH_NOVAMOON_GUI.Enabled = shouldShow
...
local AntiAdminGummy = (function()
    ...
    task.spawn(function()
        while task.wait(0.1) do
            if _GEN ~= _G._FH_NOVAMOON_GEN then break end
...
local _FH_ActiveMount  = Config.get("nm_mount_type", "Flying Carpet")
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            task.spawn(function()
                ...
                while true do
                    local need = M.nearest or M.best or M.priorityGrab or M._oneShot or (_G._FH_NovaMoonNeedList == true)
                    if not need then
                        task.wait(1)
                    else
                        task.wait(0.3)
...
do
    ...
    local grabBarLocked = Config.get("nm_grabbar_locked", true)
    ...
    barLockBtn.Activated:Connect(function()
        ...
        barLockBtn.Text       = grabBarLocked and "🔒" or "🔓"
        barLockBtn.TextColor3 = grabBarLocked and Color3.fromRGB(255, 200, 60) or T.TextMute
        Config.set("nm_grabbar_locked", grabBarLocked)
    end)
    ...
    local _barConn; _barConn = RunService.Heartbeat:Connect(LPH_NO_VIRTUALIZE(function(dt)
        if _GEN ~= _G._FH_NOVAMOON_GEN then _barConn:Disconnect(); return end
...
local function MiniPanel(title, x, y, w, extraFactor)
    ...
    local _ppKey = "nm_panelpos:" .. title
    do
    ...
    Pad(body, 6, 6, 6, 6)
    local contentH = 0
    local open = Config.get("nm_panelopen:" .. title, true)
    local curW = w
    ...
    minBtn.Activated:Connect(function()
        open = not open
        minBtn.Text = open and "-" or "+"
        Config.set("nm_panelopen:" .. title, open)
        if open then
...
task.spawn(function()
    ...
    local function buildSetPage(sk, gname, prefix, defaults)
        ...
        for _, c in ipairs(CMDS) do
            ...
            tgs[sk][c.id] = tg
            if defaults[c.id] and Config.get("nm_toggle:" .. prefix .. c.label, nil) == nil then
...
do
    ...
    local PAD  = 10
    local N    = 4
    local ubIsHorizontal = Config.get("nm_ub_horiz", true)
    ...
    local function ubToggleLayout()
        ...
        ubLayoutToggle.Text = ubIsHorizontal and "\226\134\148" or "\226\134\149"
        ubApplyLayout()
        Config.set("nm_ub_horiz", ubIsHorizontal)
...
do
    local saved = Config.get("nm_priority_set", nil)
    ...
    local function makeCard(rec)
        ...
        card.Activated:Connect(function()
            ...
            Config.set("nm_priority_set", AutoGrab.priority)
...
Input(Visual.scroll, "Friend Names", "name1, name2, ...", Config.get("nm_friendesp_names", ""), function(v)
    Config.set("nm_friendesp_names", v)
...
local function persistTheme()
    Config.set("nm_theme_c1", { math.floor(Theme.c1.R*255+0.5), math.floor(Theme.c1.G*255+0.5), math.floor(Theme.c1.B*255+0.5) })
    Config.set("nm_theme_c2", { math.floor(Theme.c2.R*255+0.5), math.floor(Theme.c2.G*255+0.5), math.floor(Theme.c2.B*255+0.5) })
    Config.set("nm_theme_bg",  { math.floor(T.Bg.R*255+0.5),    math.floor(T.Bg.G*255+0.5),    math.floor(T.Bg.B*255+0.5)    })
end
